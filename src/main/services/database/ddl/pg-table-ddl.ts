...
//
// Portions of this file are derived from chat2db (Apache License 2.0):
//   chat2db-server/chat2db-plugins/chat2db-postgresql/.../consts/SQLConst.java
// The PL/pgSQL function pg_get_tabledef is copied with minimal changes.

/**
 * Installs helper functions (pg_get_coldef, pg_get_tabledef) into the connected
 * database. Idempotent. Used by fetchPostgresTableDdl() when the helper is
 * missing. Requires CREATE FUNCTION privilege on the database.
 */
export const PG_TABLE_DEF_FUNCTION_SQL = `
...
-- Drop any stale variants that may linger from previous installs with
-- different parameter types (e.g. text vs character varying, with or
-- without the default boolean). CREATE OR REPLACE only replaces a
-- function with the *same* signature, so without these DROPs a
-- sql_identifier -> varchar lookup can resolve to an older overload
-- that PG cannot coerce (information_schema column_name is
-- sql_identifier, not text/varchar).
...
DROP FUNCTION IF EXISTS public.pg_get_coldef(text, text, text);
DROP FUNCTION IF EXISTS public.pg_get_coldef(character varying, character varying, character varying);
DROP FUNCTION IF EXISTS public.pg_get_tabledef(text, text, boolean, tabledefs[]);
